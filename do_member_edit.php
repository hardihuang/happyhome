...
<div id="do_member_info" data-role='page' data-title='修改成员信息'>
	...
	<div data-role='content'>
		...
		<?php 
		if($member_arr){
			echo '<label for="father_mid" class="select">介绍人:</label>';
			echo '<select name="father_mid" id="father_mid">';
			if(empty($father_mid)){
				echo '<option value="false" selected="selected">无</option>';
			}else{
				echo '<option value="false">无</option>';
			}
			foreach ($member_arr as $name) {
				if($father_mid==$name['mid']){
					$check= 'selected="selected" ';
				}else{
					$check='';
				}
				echo '<option value="'.$name['mid'].'" '.$check.'>'.$name['name'].'</option>';
			}
			echo '</select>';
		}
		?>
